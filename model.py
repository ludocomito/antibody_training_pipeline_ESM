...
class ESMEmbeddingExtractor:
    ...
    def __init__(self, model_name, device, batch_size=32):
        ...
        logger.info(f"ESM model {model_name} loaded on {device} with batch_size={batch_size}")
    ...
    def extract_batch_embeddings(self, sequences: List[str]) -> np.ndarray:
        ...
        logger.info(f"Extracting embeddings for {len(sequences)} sequences with batch_size={self.batch_size}...")
